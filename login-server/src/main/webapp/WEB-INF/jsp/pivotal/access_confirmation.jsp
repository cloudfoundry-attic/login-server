...
<head>
<title>Access Confirmation | Pivotal</title>
<meta charset='utf-8'>
...
<link href='${baseUrl}/stylesheets/permissions.css' media='screen'
 rel='stylesheet' type='text/css' />
<link href='${baseUrl}/stylesheets/access_confirmation.css'
  ...
  <div class="main-content access-confirmation">
   ...
   <form id="confirmationForm" name="confirmationForm"
    ...
    <div class="permissions-container">
     ...
     <p>
      ${client_id} has requested permission to access your Pivotal
      account. If you do not recognize this application or its URL (<a
       href="${redirect_uri}">${redirect_uri}</a>), you should click
      deny below. The application will not see your password.
     </p>
      ...
      <c:if test="${(! empty undecided_scopes)}">
       <c:forEach items="${undecided_scopes}" var="scope">
        <div class="approvals-list-div">
         <input type="checkbox" checked="checked" name="scope.${count}"
          id="checkbox.scope.${count}" value="${scope['code']}">
         <label for="checkbox.scope.${count}"></label>
         <spring:message code="${scope['code']}" text="${scope['text']}" />
        </div>
       ...
       <c:forEach items="${denied_scopes}" var="scope">
        <div class="approvals-list-div">
         <input type="checkbox" name="scope.${count}"
          value="${scope['code']}"> <label
          for="checkbox.scope.${count}"></label>
         <spring:message code="${scope['code']}" text="${scope['text']}" />
        </div>
        <c:set var="count" value="${count + 1}" />
       </c:forEach>
     ...
     <p>You can change your approval of permissions or revoke access
      for this application at any time from account settings. By
      approving access, you agree to ${client_id}'s terms of service and
      privacy policy.</p>
    </div>
